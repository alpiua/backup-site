...
RSERVER=""
RUSER=""
#RPASS=$(cat /root/.storagebox.pass)
...
if [ $returncode -ne 0 ] 
  then
    ...
    echo "Error occured while uploading "$1" to external share" | mail -s "Error while uploading "$site" backup" $ATTACHMENT" $email"
  else
...
[ ${FREE_SPACE} -lt ${SITE_SIZE} ] && log "Not enough free space on local host. Removing old backups" && delete_old "$WORKDIR" 
